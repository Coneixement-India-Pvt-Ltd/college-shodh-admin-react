...
//                       <Button className="text-white rounded-lg p-2 w-full"
...
<<<<<<< HEAD
const Card = ({ naacFilter, stateFilter, nbaFilter }) => {
    const [colleges, setColleges] = useState([]);
    const [search, setSearch] = useState("");
    const [selectedState, setSelectedState] = useState("");
    const [selectedCourse, setSelectedCourse] = useState("");
    ...
    const filteredColleges = colleges.filter((college) => {
      const isStateMatch =
        ...
        (college.address && stateFilter.includes(college.address.toLowerCase()));
      const isNaacMatch =
        naacFilter.length === 0 ||
        (college.naac && naacFilter.includes(college.naac));
      const isNbaMatch =
        nbaFilter.length === 0 ||
        (college.nba && nbaFilter.includes(college.nba));
      const isCourseMatch = selectedCourse
        ? college.course &&
          college.course.toLowerCase().includes(selectedCourse.toLowerCase())
        : true;
      ...
      return isStateMatch && isNaacMatch && isNbaMatch && isCourseMatch && isSearchMatch;
    });
